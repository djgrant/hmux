...
if (!process.stdout.isTTY) {
  console.error(
    "hmux: stdout is not a terminal — refusing to render a clamped UI.\n" +
...
if (arg === "target") {
  ...
  if (!tty || tty === "not a tty") {
    console.error("hmux target: cannot determine this terminal's tty.")
    process.exit(1)
  }
  ...
  if (existing.length > 0) {
    process.stdout.write(
      `hmux target: ${existing[0].tty} is already the target. [k]ill it and take over, or [a]bort? `,
    )
    ...
    if (answer.toLowerCase() !== "k") {
      console.error("hmux target: aborted — existing target kept.")
      process.exit(1)
    }
    // Detaching the old client ends its `hmux target` process (its attach
  ...
  Bun.spawnSync([
    ...
    "stty -echo; printf '\\n  hmux target — pick a session in hmux to load it here\\n'; exec cat >/dev/null",
  ])
  registerTarget(tty, process.env.TERM_PROGRAM ?? null)
  // `hmux target --cc` attaches in control mode: this tab becomes the tmux
