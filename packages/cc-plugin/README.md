...
All five hooks run one script, [`src/hook.ts`](src/hook.ts), which reads the
...
| `UserPromptSubmit` | Marks the session `working`, then checks the session's `bound` flag (`GET /sessions/:id`). If the human has bound the session from the TUI, it injects `additionalContext` telling the agent to use `mcp__humans__ask` for questions/decisions and `mcp__humans__notify` for progress. |
