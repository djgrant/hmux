...
| `PreToolUse` | For `mcp__humans__*` calls only: stamps the harness-provided session id into the tool input, so message attribution is harness-asserted. Also the mux listening post — `ask`/`approve` advertise a `message` label into the pane for the duration of the block, and `signal` stashes the declared turn-end status for `Stop` to promote. |
| `Stop` | Promotes a `signal` declared this turn into a mux `message` label — `question` → "needs direction" (roster `needs-attention`), `done` → "done" — otherwise the session rests at `idle` with no label. |
