...
import { DEFAULT_PORT, WS_PATH, type Message, type ServerEvent, type Session } from "@hmux/protocol"

const now = Date.now()
const messages: Message[] = [
  { id: "m1", kind: "ask", agent: "refactor-bot", project: "hmux", body: "Rename `Queue` to `Inbox`? It collides with the data structure.", status: "pending", createdAt: now - 2 * 60_000 },
  { id: "m2", kind: "ask", agent: "deploy-bot", project: "hmux", body: "Ship v0.0.1 to npm now, or wait for the docs PR?", context: "CI is green on main.", suggestion: "ship it now", status: "pending", createdAt: now - 60 * 60_000 },
  { id: "m3", kind: "ask", agent: "scraper", body: "Rate limit hit on API — back off for 1h?", status: "pending", createdAt: now - 5 * 60_000 },
  { id: "m4", kind: "ask", agent: "docs-bot", project: "hmux", body: "Use **bold** headings and `inline code` in the README?\n\n- option a\n- option b", status: "answered", answer: "Yes — go with **option a**.", createdAt: now - 3 * 60 * 60_000, answeredAt: now - 2 * 60 * 60_000 },
  { id: "m5", kind: "notify", agent: "ci-bot", project: "hmux", body: "Build passed on `main` (2m14s).", status: "pending", createdAt: now - 30_000 },
  // Approvals: permission prompts from headless agents (their own section).
  { id: "m6", kind: "approval", agent: "headless-1", project: "hmux", body: "wants to run Bash\n\n```json\n{\n  \"command\": \"rm -rf dist\"\n}\n```", suggestion: "allow", status: "pending", createdAt: now - 20_000 },
  { id: "m7", kind: "approval", agent: "headless-1", project: "hmux", body: "wants to run Edit\n\n```json\n{\n  \"file_path\": \"release.ts\"\n}\n```", suggestion: "allow", status: "pending", createdAt: now - 10_000 },
]
...
const sessions: Session[] = [
  { id: "s1", agent: "refactor-bot", project: "hmux", model: "fable", status: "working", bound: false, startedAt: now - 10 * 60_000, lastSeen: now },
  { id: "s2", agent: "deploy-bot", project: "hmux", status: "idle", bound: true, startedAt: now - 90 * 60_000, lastSeen: now - 20_000 },
  { id: "s3", agent: "ci-bot", project: "hmux", status: "needs-attention", detail: "Claude needs your permission to use Bash", bound: false, startedAt: now - 40 * 60_000, lastSeen: now },
  ...
  { id: "s5", agent: "prover", project: "hmux", status: "working", bound: false, startedAt: now - 5 * 60_000, lastSeen: now - 10 * 60_000 },
]
